...
parser.add_argument("-a","--noise", type = float, default = 0)
parser.add_argument("-i","--weight_tau", type = bool, default=False)
parser.add_argument("-j","--tau",type=float,default=0)
parser.add_argument("-o",'--agg_method',type=str, default='mean')
args = parser.parse_args()
if args.agg_method == 'cosine':
    print('cos')
else:
...
data =utils.set_noise(data,NOISE_PERCENTAGE)
...
def get_target_dataset(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    target_da_metatrain_data = utils.set_noise(target_da_train_set, NOISE_PERCENTAGE)
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, con_features, labels):  # emb_i, emb_j 是来自同一图像的两种不同的预处理方法得到
        representations = F.normalize(con_features, dim=1)
        ...
        loss_partial = -torch.log(nominator / (denominator + 1e-8)) # 2*bs
        loss = torch.sum(mask * loss_partial) / (mask.sum() + 1e-8)
        return loss


class ContrastiveLossWeight(nn.Module):
    def __init__(self,  temperature = 0.5):
        super().__init__()
        self.temperature = temperature

    def forward(self, con_features, labels, weights):  # emb_i, emb_j 是来自同一图像的两种不同的预处理方法得到
        representations = F.normalize(con_features, dim=1)
        similarity_matrix = euclidean_metric(representations, representations) # simi_mat: (2*bs, 2*bs)
        ...
        for i in range(num_sample):
            for j in range(num_sample):
                if labels[i] == labels[j] and weights[i] > pair_positive:
                    mask[i, j] = 1.0
                else:
                    mask[i,j] = 0.0
...
# infoNCE_Loss = ContrastiveLoss(batch_size = CLASS_NUM).to(device)
# run 10 times
nDataSet = 10
...
# seeds = [1337, 1233, 1330, 1231, 1336, 1224, 1233, 1226, 1235, 1229]
# seeds = [1330]
seeds = [1330, 1220, 1336, 1337, 1224, 1236, 1226, 1235, 1233, 1229]
# seeds = [1336, 1337, 1231,1330,  1236, 1226, 1235, 1233, 1229, 1347]
for iDataSet in range(nDataSet):
    ...
    for episode in range(3001):  # EPISODE = 90000
        ...
        if episode %2 ==0:
            ...
            # sample datas
            supports, support_labels = support_dataloader.__iter__().__next__()  # (80,128,9,9)
            querys, query_labels = query_dataloader.__iter__().__next__()  # (240,128,9,9)
            ...
            if AGG_METHOD == 'mean':
                # print('this is mean')
                query_proto = utils.gen_prototypes_one(query_features, CLASS_NUM, QUERY_NUM_PER_CLASS,
                ...
                #                 support_proto = utils.gen_prototypes_one(support_features, CLASS_NUM, SHOT_NUM_PER_CLASS,
                #
                logits_pro = euclidean_metric(support_features, query_proto)
                proto_loss = crossEntropy(logits_pro, labels_proto)
            else:
                # print('this is cosine')
                query_proto, query_weights = utils.gen_prototypes_weights(query_features, CLASS_NUM, QUERY_NUM_PER_CLASS,
                                                              agg_method=AGG_METHOD)
                support_proto, support_weights = utils.gen_prototypes_weights(support_features, CLASS_NUM,
                                                                              SHOT_NUM_PER_CLASS,
                                                                   agg_method=AGG_METHOD)
                # s-fsl_loss
                ...
                proto_loss = crossEntropy(logits_pro, labels_proto)
                # proto_loss = proto_loss / (2 * CLASS_NUM)

            # cl loss
            if AGG_METHOD != 'mean' and args.weight_tau:
                cl_loss = infoNCE_Loss(support_features, labels_proto, support_weights)
            else:
                cl_loss = infoNCE_Loss(support_features, labels_proto)
            loss = cl_loss + proto_loss + f_loss
        ...
        # target domain few-shot + domain adaptation
            ...
            if AGG_METHOD == 'mean':
                ...
                query_proto, query_weights = utils.gen_prototypes_weights(query_features, CLASS_NUM,
                                                                          QUERY_NUM_PER_CLASS,
                                                                          agg_method=AGG_METHOD)
                support_proto, support_weights = utils.gen_prototypes_weights(support_features, CLASS_NUM,
                                                                              SHOT_NUM_PER_CLASS,
        ...
        if (episode + 1) % 100 == 0:  # display
            ...
            print(
                'episode {:>3d}:  q-fsl loss: {:6.4f}, s-fsl loss: {:6.4f} , contrastive loss: {:6.4f} , acc {:6.4f}, loss: {:6.4f}'.format(
                    episode + 1, \
                    proto_loss.item(),
                    f_loss.item(),
                    cl_loss.item(),
                    2 * total_hit / total_num,
                    loss.item()))
        if (episode + 1) % 100 == 1:  # display
            train_loss.append(loss.item())
            print(
                'episode {:>3d}:  q-fsl loss: {:6.4f}, s-fsl loss: {:6.4f} , contrastive loss: {:6.4f} , acc {:6.4f}, loss: {:6.4f}'.format(
                    episode + 1, \
        ...
        if (episode + 1) % 200 == 1 or episode == 0:
            ...
            if test_accuracy > last_accuracy:
                ...
                torch.save(feature_encoder.state_dict(),
                           str("checkpoints/SA/" + current_file_name   + str(iDataSet) + "iter_" + str(
...
utils.classification_map(hsi_pic[4:-4, 4:-4, :], best_G[4:-4, 4:-4], 24,
                         logging_file_dic + "/SA_{}shot.png".format(TEST_LSAMPLE_NUM_PER_CLASS))
sys.stdout.close()


# if __name__ == '__main__':
#     print('python WCPN-SA.py -a 0.0')
    # os.system("python WCPN-SA.py -a 0.0")
    # print('python WCPN-SA.py -a 0.2')
    # os.system("python WCPN-SA.py -a 0.2")
    # print('python WCPN-SA.py -a 0.4')
    # os.system("python WCPN-SA.py -a 0.4")
    # print('python WCPN-SA.py -a 0.6')
    # os.system("python WCPN-SA.py -a 0.6")
    # print('python WCPN-SA.py -a 0.8')
    # os.system("python WCPN-SA.py -a 0.8")